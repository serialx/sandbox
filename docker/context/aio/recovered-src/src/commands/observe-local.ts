...
// src/commands/observe-local.ts
import { execFileSync } from "node:child_process";
import { readFileSync as readFileSync2 } from "node:fs";
import { userInfo } from "node:os";
import { setTimeout as sleep } from "node:timers/promises";
var DEFAULT_INTERVAL_SECONDS = 2;
var DEFAULT_HISTORY_POINTS = 30;
var DEFAULT_TOP_ROWS = 8;
var DEFAULT_WARMUP_MS = 250;
var DEFAULT_CLOCK_TICKS_PER_SECOND = 100;
var PS_ARGS = ["-eo", "pid=,user=,ppid=,rss=,comm="];
var CLEAR_SCREEN = "\x1B[2J\x1B[H";
var UNKNOWN_BAR_CHAR = "?";
var FILLED_BAR_CHAR = "#";
var EMPTY_BAR_CHAR = "-";
function createObserveLocalCommand(inSandbox, runObserveLocalImpl = runObserveLocal) {
...
async function runObserveLocal(options) {
...
function createHistoryState(maxPoints) {
  return {
    cgroupMemoryMiB: [],
    cgroupUsagePct: [],
...
function renderObserveLocalDashboard(snapshot, history, options) {
...
function calculateCpuPct(pid, cpuTicksByPid, state, elapsedSeconds) {
...
function ensureSupportedEnvironment(inSandbox) {
...
function readCpuTicksByPid(pids) {
...
function buildCgroupFileCandidates(v2Name, v1Name) {
  const candidates = [];
...
function resolveCgroupBasePaths() {
  const basePaths = [];
  try {
...
function formatCgroupUsage(snapshot) {
...
function calculateProcessMemoryPct(rssKb, maxBytes) {
  if (maxBytes === null || maxBytes <= 0) {
    return null;
  }
  return rssKb * 1024 / maxBytes * 100;
}
function renderPercentBar(pct, width) {
  if (width <= 0) {
    return "[]";
  }
...
function classifyUsageLevel(pct) {
  if (pct === null) {
    return "n/a";
  }
  if (pct >= 90) {
...
function summarizeTrend(values, unitSuffix, flatThreshold) {
  if (values.length === 0) {
    return {
      direction: "n/a",
      deltaText: "n/a",
...
function formatSignedValue(value, unitSuffix) {
  const sign = value >= 0 ? "+" : "-";
  return `${sign}${Math.abs(value).toFixed(1)}${unitSuffix}`;
}
function formatUnsignedValue(value, unitSuffix) {
...
function pushBounded(values, value, maxPoints) {
  values.push(value);
