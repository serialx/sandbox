...
import { createObserveLocalCommand } from "./observe-local.js";
import { getClient } from "../index.js";
import { detectFormat, printResponse } from "../output.js";
export function parseDurationSeconds(raw, createCliError) {
...
export function printObservationStatus(status) {
...
export function printObservationSnapshot(snapshot) {
...
export function printObservationStart(result) {
...
export function printObservationStop(result) {
  console.log(`session_id: ${result.session_id}`);
  console.log(`stopped: ${result.stopped}`);
  console.log(`report_ready: ${result.report_ready}`);
}
export function printObservationReports(reports) {
  if (reports.length === 0) {
...
export function attachObservationCommands(sandbox, deps) {
