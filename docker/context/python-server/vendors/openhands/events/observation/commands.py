...
from vendors.openhands.core.schema import ObservationType
from vendors.openhands.events.observation.observation import Observation
...
class CmdOutputMetadata(BaseModel):
    ...
    working_dir: str | None = None  # Physical path (symlinks resolved)
    working_dir_symlink: str | None = None  # Raw $PWD (symlinks preserved)
    ...
    def to_ps1_prompt(cls) -> str:
        ...
        Both working_dir (resolved) and working_dir_symlink (raw $PWD) are included,
        allowing the caller to choose which one to use at runtime based on
        preserve_symlinks setting.
        """
        ...
        json_str = json.dumps(
            {
                ...
                'username': r'\u',
                'hostname': r'\h',
                'working_dir': r'$(pwd)',  # Physical path (symlinks resolved)
                'working_dir_symlink': r'$PWD',  # Raw $PWD (symlinks preserved)
