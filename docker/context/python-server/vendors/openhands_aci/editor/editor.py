...
from app.utils import is_binary
from openhands_aci.editor.config import SNIPPET_CONTEXT_WINDOW
from vendors.openhands_aci.editor.encoding import EncodingManager, with_encoding
...
from vendors.openhands_aci.editor.md_converter import MarkdownConverter
...
from vendors.openhands_aci.editor.replacer import ReplaceMode, do_replacement
from vendors.openhands_aci.editor.results import CLIResult, maybe_truncate
...
class OHEditor:
    ...
    def __call__(
        ...
        *,
        ...
        replace_mode: ReplaceMode | None = None,
        ...
        **kwargs,
        ...
        if command == 'view':
            ...
            return self.str_replace(_path, old_str, new_str, enable_linting, replace_mode)
        elif command == 'insert':
    ...
    def str_replace(
        ...
        Args:
            ...
            replace_mode: Replacement mode ('ALL', 'FIRST', 'LAST'). If None, requires unique match.
        ...
        # When replace_mode is specified, use the new replacement logic
        if replace_mode is not None:
            result = do_replacement(file_content, old_str, new_str, replace_mode)
            ...
            # Prepare the success message with diff
            success_message = f'The file {path} has been edited. {replacements} replacement(s) made using mode {replace_mode}.\n'
            if result['diff_text']:
                success_message += f"Diff:\n{result['diff_text']}\n"
        ...
        # Original behavior: require unique match when replace_mode is None
    ...
    def view(
        ...
        Args:
            ...
            view_range: Line range for text files [start, end] (1-indexed)
            page_range: Page range for PDF files [start, end] (1-indexed)
            sheet_name: Sheet name for Excel files
            row_range: Row range for Excel files [start, end] (1-indexed)
            slide_range: Slide range for PPTX files [start, end] (1-indexed)
            enable_metadata: Whether to return file metadata
        """
        if path.is_dir():
            if view_range:
                raise EditorToolParameterInvalidError(
        ...
        if self.is_supported_binary_file(path):
            result = self.read_file_markdown(
                path,
                page_range=page_range,
                sheet_name=sheet_name,
                row_range=row_range,
            ...
            return CLIResult(
                output=self._make_output(
                    result.text_content, str(path), 1, is_converted_markdown=True
                ),
                path=str(path),
                prev_exist=True,
        ...
        if not view_range:
            ...
            return CLIResult(
                output=output, path=str(path), prev_exist=True, old_content=file_content
            )
        ...
        return CLIResult(
            path=str(path), output=output, prev_exist=True, old_content=file_content
        )
    ...
    def write_file(self, path: Path, file_text: str, encoding: str = 'utf-8') -> None:
        ...
        try:
            ...
            raise ToolError(f'Ran into {e} while trying to write to {path}') from e
    ...
    def read_file(
        ...
        try:
            ...
            raise ToolError(f'Ran into {e} while trying to read {path}') from e
    ...
    def read_file_markdown(
        ...
        """
        Read and convert a binary file to Markdown.
        ...
        Returns:
            DocumentConverterResult with text_content and optional metadata
        """

        try:
            result = self._markdown_converter.convert(
                str(path),
                page_range=page_range,
                sheet_name=sheet_name,
                row_range=row_range,
