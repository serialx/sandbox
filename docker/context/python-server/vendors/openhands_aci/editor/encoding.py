...
This is a patched version of openhands_aci.editor.encoding that fixes
the bug where `results['confidence']` can be None, causing a TypeError
when comparing with the confidence threshold.
"""
...
class EncodingManager:
    ...
    def detect_encoding(self, path: Path) -> str:
        ...
        # FIX: Check that confidence is not None before comparing
        confidence = results.get('confidence') if results else None
        if results and confidence is not None and confidence > self.confidence_threshold:
