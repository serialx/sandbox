# This is copied from Magentic-one's repo: https://github.com/microsoft/autogen/blob/v0.4.4/python/packages/autogen-magentic-one/src/autogen_magentic_one/markdown_browser/mdconvert.py
# type: ignore
import base64
import copy
import html
...
class _CustomMarkdownify(markdownify.MarkdownConverter):
    ...
    def convert_img(self, el: Any, text: str, parent_tags: Any = None) -> str:
...
class PdfConverter(DocumentConverter):
    ...
    Supports pagination via page_range parameter and metadata extraction.
    """
    ...
    def convert(self, local_path, **kwargs) -> Union[None, DocumentConverterResult]:
        ...
        enable_metadata = kwargs.get('enable_metadata', False)
        ...
        if page_range and len(page_range) == 2:
            ...
            current_range = [start_page, end_page]

        # Extract text (with optional page filter)
        text_content = high_level.extract_text(local_path, page_numbers=page_numbers)
...
class DocxConverter(HtmlConverter):
    ...
    def convert(self, local_path, **kwargs) -> Union[None, DocumentConverterResult]:
        ...
        metadata = None
        if enable_metadata:
            # Get paragraph count using python-docx
...
class XlsxConverter(HtmlConverter):
    ...
    Supports sheet selection, row range pagination, and metadata extraction.
    """
    ...
    def convert(self, local_path, **kwargs) -> Union[None, DocumentConverterResult]:
        ...
        # First, get all sheet names for metadata
        ...
        if sheet_name:
            ...
            read_params['sheet_name'] = None  # Read all sheets

        if row_range and len(row_range) == 2:
            start_row, end_row = row_range
            read_params['skiprows'] = range(1, start_row) if start_row > 1 else None
            read_params['nrows'] = end_row - start_row + 1

        sheets = pd.read_excel(local_path, **read_params)

        # Handle single sheet vs multiple sheets
        if isinstance(sheets, pd.DataFrame):
            # Single sheet returned
            sheets = {sheet_name or all_sheets[0]: sheets}
        ...
        for s in sheets:
            ...
            if sheet_name:
                current_sheet = s
                total_rows = len(pd.read_excel(local_path, sheet_name=s))  # Full row count

        # Build metadata if requested
        metadata = None
        if enable_metadata:
            metadata = {
                'file_type': 'xlsx' if extension.lower() == '.xlsx' else 'xls',
                'file_path': os.path.abspath(local_path),
                'file_size': os.path.getsize(local_path),
                'sheets': all_sheets,
                'current_sheet': current_sheet,
                'total_rows': total_rows,
                'current_row_range': row_range if row_range else None,
            }
...
class PptxConverter(HtmlConverter):
    ...
    Supports slide range pagination and metadata extraction.
    """
    ...
    def convert(self, local_path, **kwargs) -> Union[None, DocumentConverterResult]:
        ...
        if slide_range and len(slide_range) == 2:
            start_slide = max(1, slide_range[0])
        ...
        if enable_metadata:
            metadata = {
                ...
                'current_slide_range': [start_slide, end_slide] if slide_range else None,
            }
